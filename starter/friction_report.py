...
in `friction_verdicts.py`, so the line between what the data says and where
...
<code>friction.py</code> and <code>execution.py</code> · thresholds in <code>friction_verdicts.py</code> · {pd.Timestamp.now('UTC'):%Y-%m-%d %H:%M} UTC</footer>""")
